...
impl Report {
    ...
    fn is_safe(&self) -> bool {
        ...
        self.get_adj_difference().iter().all(|&x| x >= 1 && x <= 3)
    }
...
1 3 6 7 9";
    ...
    #[test]
    fn test_is_safe() {
        assert_eq!(parse(EXAMPLE)[0].is_safe(), true);
        assert_eq!(parse(EXAMPLE)[1].is_safe(), false);
        assert_eq!(parse(EXAMPLE)[2].is_safe(), false);
        assert_eq!(parse(EXAMPLE)[3].is_safe(), false);
        assert_eq!(parse(EXAMPLE)[4].is_safe(), false);
        assert_eq!(parse(EXAMPLE)[5].is_safe(), true);
    }
    ...
    #[test]
    fn test_solve_2() {
        assert_eq!(parse(EXAMPLE)[0].is_safe_more_tolerant(), true);
        assert_eq!(parse(EXAMPLE)[1].is_safe_more_tolerant(), false);
        assert_eq!(parse(EXAMPLE)[2].is_safe_more_tolerant(), false);
        assert_eq!(parse(EXAMPLE)[3].is_safe_more_tolerant(), true);
        assert_eq!(parse(EXAMPLE)[4].is_safe_more_tolerant(), true);
        assert_eq!(parse(EXAMPLE)[5].is_safe_more_tolerant(), true);
    }
