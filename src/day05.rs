...
fn check_if_sorted(pages: &Vec<u32>, order: &HashSet<(u32, u32)>) -> bool {
...
fn solve_1(pages: &Vec<Vec<u32>>, order: &HashSet<(u32, u32)>) -> u32 {
    pages
        .iter()
...
fn solve_2(pages: &Vec<Vec<u32>>, order: &HashSet<(u32, u32)>) -> u32 {
    pages
        .iter()
        .filter(|x| !check_if_sorted(x, &order))
        .map(|x| {
...
97,13,75,29,47";
    ...
    fn test_is_sorted() {
        ...
        assert_eq!(check_if_sorted(&pages[0], &order), true);
        assert_eq!(check_if_sorted(&pages[1], &order), true);
        assert_eq!(check_if_sorted(&pages[2], &order), true);
        assert_eq!(check_if_sorted(&pages[3], &order), false);
        assert_eq!(check_if_sorted(&pages[4], &order), false);
        assert_eq!(check_if_sorted(&pages[5], &order), false);
    }
}
